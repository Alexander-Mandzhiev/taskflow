...
// GetByID возвращает задачу по id. GET /api/v1/tasks/{id}. Доступ только для участников команды задачи.
func (api *API) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	task, err := api.taskService.GetByID(r.Context(), taskID, userID)
	if err != nil {
		mapError(w, r, err)
		return
	}
