...
// ListComments возвращает комментарии к задаче. GET /api/v1/tasks/{id}/comments.
func (api *API) ListComments(w http.ResponseWriter, r *http.Request) {
	...
	comments, err := api.commentService.ListByTaskID(r.Context(), taskID, userID)
	if err != nil {
		mapError(w, r, err)
		return
	}

	pkghttp.WriteJSON(r.Context(), w, http.StatusOK, taskconverter.CommentsToResponse(comments))
}
