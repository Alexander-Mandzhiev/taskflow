...
type TeamReaderRepository interface {
	...
	GetByID(ctx context.Context, tx *sqlx.Tx, teamID string) (*model.Team, error)
...
type TeamWriterRepository interface {
	...
	Create(ctx context.Context, tx *sqlx.Tx, input *model.TeamInput, createdBy string) (*model.Team, error)
}
