...
// toDomainError преобразует ошибку БД в доменную ошибку модуля team (для операций чтения приглашений).
func toDomainError(err error) error {
	if err == nil {
		return nil
	}
	if errors.Is(err, model.ErrInvitationNotFound) ||
		errors.Is(err, model.ErrTeamNotFound) ||
