...
type TeamReaderRepository interface {
	...
	GetByID(ctx context.Context, tx *sqlx.Tx, teamID string) (*model2.Team, error)
	...
	ListByUserID(ctx context.Context, tx *sqlx.Tx, userID string) ([]*model2.TeamWithRole, error)
}
...
type TeamWriterRepository interface {
	...
	Create(ctx context.Context, tx *sqlx.Tx, input *model2.TeamInput, createdBy string) (*model2.Team, error)
}
