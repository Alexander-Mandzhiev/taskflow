...
func (s *ServiceSuite) TestInviteByEmail_Success() {
	...
	s.invitationRepo.On("CreateInvitation", mock.Anything, mock.Anything, mock.MatchedBy(func(inv model.TeamInvitation) bool {
		return inv.TeamID == teamID && inv.Email == inviteeEmail && inv.Role == model.RoleMember && inv.InvitedBy == inviterID
	})).Return(nil).Once()
...
func (s *ServiceSuite) TestInviteByEmail_Forbidden_NotOwnerOrAdmin() {
	...
	assert.Equal(s.T(), model.TeamInvitation{}, inv)
	s.teamRepo.AssertExpectations(s.T())
	s.memberRepo.AssertExpectations(s.T())
	s.invitationRepo.AssertExpectations(s.T())
}
...
func (s *ServiceSuite) TestInviteByEmail_Success_UserRegisteredNotInTeam() {
	...
	s.invitationRepo.On("GetPendingInvitationByTeamAndEmail", mock.Anything, mock.Anything, teamID, inviteeEmail).
		Return(model.TeamInvitation{}, model.ErrInvitationNotFound).Once()
	s.invitationRepo.On("CreateInvitation", mock.Anything, mock.Anything, mock.MatchedBy(func(inv model.TeamInvitation) bool {
		return inv.TeamID == teamID && inv.Email == inviteeEmail && inv.InvitedBy == inviterID
	})).Return(nil).Once()
